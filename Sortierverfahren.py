...
def merge(a, b):
    result = []
    while (len(a) != 0) and (len(b) != 0): #Zusammenführen bis eine Liste leer ist
        if a[0] < b[0]: #Anhängen des kleineren der beiden Elemente an das Ergebnis
            result.append(a[0])
            a = a[1:len(a)] #Erstes Element von a entfernen
        else:
            result.append(b[0])
            b = b[1:len(b)] #Erstes Element von b entfernen
    if a != []: #Falls a noch nicht leer ist
        result = result + a #Anhängen von a an das Ergebnis
    if b != []: #Falls b noch nicht leer ist
        result = result + b #Anhängen von b an das Ergebnis
    return result
...
def sortHeapDesc(a):
    ...
    while l > 0:
        a = swap(a, 0, l)
        print("Sortierschritt: ", a)
        l -= 1 #Letztes Element gehört nicht mehr zum Baum
...
def heapify(a, p, l): #p: Knoten für den Heapeigenschaft hergestellt wird, l: Ende des Baums
    if 2*p+1 > l: #p hat kein Kind, daher ist nichts zu machen
        return a
    ...
    if a[2*p+1] < a[2*p+2]: #Bestimmen des kleineren Kindes von p
        if a[2*p+1] < a[p]: #Wenn Kind kleiner als Vater ist
